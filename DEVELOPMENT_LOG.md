...
## 12. MLX Whisper 替换 faster-whisper

### 动机
faster-whisper 使用 CTranslate2 引擎，不支持 Apple Silicon GPU（Metal），Mac 上只能跑 CPU。small 模型在 CPU 上的准确率只有 78%（字符级）。

### 发现
MLX Whisper 利用 Apple Silicon 的 Metal GPU 加速，能在 Mac 本地跑 large-v3-turbo（和 5090 相同的模型）。
...
MLX Whisper 比 faster-whisper 高 13 个百分点。初次评估时 word-level overlap 显示 MLX 只有 5%——这是评估 bug：MLX 输出缺标点导致分词全错。改用字符级重叠后发现实际 91%。

### 实施
全面替换 faster-whisper 为 mlx-whisper：
- `stt/whisper_local.py` 重写为 MLX 后端
- `config.whisper_model` 改为 `mlx-community/whisper-large-v3-turbo`
- `requirements.txt` 中 `faster-whisper` 替换为 `mlx-whisper`
- `transcribe.py` 本地模式改用 MLX
- `--local` flag 现在跑 MLX Whisper（Mac GPU），不再是 faster-whisper small（CPU）
